#!/bin/zsh
## 64位server启动命令
java -d64 -XX:+UseG1GC -Xms1g -Xmx1g -XX:MaxGCPauseMillis=200 -jar qiuqiu-admin-0.0.1-SNAPSHOT.jar >/dev/null 2>&1 &

## 64位server启动命令(包含gc日志)
java -d64 -XX:+UseG1GC -Xms4g -Xmx4g -XX:MaxGCPauseMillis=200 -XX:+PrintGCTimeStamps -XX:+PrintGCDetails -Xloggc:/tmp/gc/qiuqiu-admin-gc.log -XX:+UseGCLogFileRotation -XX:NumberOfGCLogFiles=50 -XX:GCLogFileSize=10m -jar qiuqiu-admin-0.0.1-SNAPSHOT.jar >/dev/null 2>&1 &
