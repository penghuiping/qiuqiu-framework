...
public class Validator {
    ...
    public static boolean isUpperCase(CharSequence value) {
        AssertUtil.notNull(value, "value不能为null");
    ...
    public static boolean isNumber(String value) {
        AssertUtil.hasText(value, "value不能为空");
        for (int i = 0; i < value.length(); i++) {
    ...
    public static boolean isBirthday(int year, int month, int day) {
        ...
        if (month == 2) {
            if (day > 29 || (day == 29 && false == TimeUtil.isLeapYear(year))) {
                return false;
            }
        }
        return true;
    ...
    public static boolean isChinese(CharSequence value) {
        return isMatchRegex("^" + Constant.RE_CHINESE + "+$", value);
    }
    ...
    public static boolean isBetween(Number value, Number min, Number max) {
        AssertUtil.notNull(value,"value不能为null");
        AssertUtil.notNull(min,"min不能为null");
        AssertUtil.notNull(max,"max不能为null");
