...
public class IdcardUtil {
    ...
    public static String convert15To18(String idCard) {
        ...
        if (Validator.isNumber(idCard)) {
            ...
            Date birthDate = TimeUtil.parseDate(birthday, DateTimeFormatter.ofPattern("yyMMdd"));
            // 获取出生年(完全表现形式,如：2010)
            int sYear = TimeUtil.toLocalDateTime(birthDate).getYear();
            if (sYear > 2000) {
    ...
    public static Date getBirthDate(String idCard) {
        ...
        return null == birthByIdCard ? null : TimeUtil.parseDate(birthByIdCard, DateTimeFormatter.ofPattern("yyyyMMdd"));
    }
    ...
    public static int getGenderByIdCard(String idCard) {
        AssertUtil.hasText(idCard, "身份证号不能为空");
